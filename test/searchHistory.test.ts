...
describe('persistent search history', () => {
    it('creates a JSON state in the adapter namespace without resetting it on restart', async () => {
        ...
        expect(await history.get()).to.deep.equal({ entries: [], lastSearch: '' });
        ...
        expect(await new SearchHistory(db.adapter).get()).to.deep.equal({
            entries: ['mqtt.0'],
            lastSearch: 'mqtt.0',
        });
        expect(db.write.lastCall.args).to.deep.equal([
            'searchHistory',
            { val: '{"entries":["mqtt.0"],"lastSearch":"mqtt.0"}', ack: true },
        ]);
    ...
    it('keeps the newest ten and moves case-insensitive duplicates to the front', async () => {
        ...
        const snapshot = await history.remember('TERM 5');
        expect(snapshot.entries).to.deep.equal([
            'TERM 5',
            'term 11',
            'term 10',
            'term 9',
        ...
        expect(snapshot.lastSearch).to.equal('TERM 5');
        await history.remember('TERM 5');
        expect(db.write.callCount).to.equal(14); // initialization + 12 queries + one promotion
    });

    it('ignores invalid input and excludes empty terms while preserving literal spaces and Unicode', async () => {
        const history = new SearchHistory(storage().adapter);
        for (const input of ['', '  ', null, undefined, 5, {}, ['bad']]) {
            expect(await history.remember(input)).to.deep.equal({ entries: [], lastSearch: '' });
        }
        expect(await history.remember(' äöü 雪 ')).to.deep.equal({
            entries: [' äöü 雪 '],
            lastSearch: ' äöü 雪 ',
        });
    });

    it('remembers a cleared filter without removing non-empty history entries', async () => {
        const history = new SearchHistory(storage().adapter);
        await history.remember('needle');
        expect(await history.remember('')).to.deep.equal({
            entries: ['needle'],
            lastSearch: '',
        });
    ...
    it('recovers from malformed state values and normalizes stored arrays', async () => {
        for (const val of ['not json', '{}', 'null', 12]) {
            ...
            expect(await history.get()).to.deep.equal({ entries: [], lastSearch: '' });
            expect(await history.remember('next')).to.deep.equal({
                entries: ['next'],
                lastSearch: 'next',
            });
        }
    ...
    it('does not report failed writes as saved and retries on the next request', async () => {
        ...
        expect(await history.remember('retry')).to.deep.equal({
            entries: ['retry', 'old'],
            lastSearch: 'retry',
        });
    });
