...
function parseLogLine(line) {
    ...
    const [, ts, level, source, _pid, message] = match;
 ...
 * @returns {Promise<{ok: true, rows: Array<{ts: string, level: string, source: string, message: string, raw: string, rawPlain: string}>, total: number, truncated: boolean} | {ok: false, error: string}>}
 */
async function searchLogs(options) {
    ...
    for (const name of candidates) {
        ...
        for (let i = lines.length - 1; i >= 0; i--) {
            ...
            if (searchNeedle && !parsed.rawPlain.toLowerCase().includes(searchNeedle) && !parsed.raw.toLowerCase().includes(searchNeedle)) {
