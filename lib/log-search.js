...
async function streamLogLines(filePath, isGzip, startOffset, onLine, endOffset) {
    ...
    const input = fsSync.createReadStream(filePath, streamOptions && Object.keys(streamOptions).length ? streamOptions : undefined);
...
async function searchLogs(options) {
    ...
    if (activeOnly) {
        ...
        if (startOffset >= snapshotSize) {
            emitDiagnostics();
            return { ok: true, rows: [], total: 0, truncated: false, cursor: createCursor(activeFile, snapshotStats, initialLineNumber) };
        }
