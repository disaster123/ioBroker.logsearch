...
/**
 * @typedef {{ts: string, level: string, source: string, message: string, raw: string, rawPlain: string}} LogRow
 */
...
async function searchLogs(options) {
    ...
    if (activeOnly) {
        ...
        const initialLineNumber = startOffset > 0 && cursor ? Math.max(0, Math.floor(Number(cursor.lineNumber) || 0)) : 0;
        ...
        if (activeFile) {
            try {
                const stats = await fs.stat(path.join(logDirectory, activeFile));
                responseCursor = createCursor(activeFile, stats, 0);
            } catch {
                responseCursor = null;
            }
        }
