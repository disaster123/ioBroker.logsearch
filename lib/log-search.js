 ...
 * @property {string} file Active log file name.
 * @property {number} byteOffset Next byte offset to read.
 * @property {number} lineNumber Last processed physical line number.
 * @property {number} size File size at the time the cursor was created.
 * @property {number} mtimeMs File modification time at the time the cursor was created.
 */

/**
 * @typedef {object} SearchLogsOptions
 * @property {string} logDirectory Directory containing ioBroker log files.
 * @property {string} [searchText] Case-insensitive text filter.
 * @property {number} [hours] Number of recent hours to search.
 * @property {"all"|"error"|"warn"|"info"|"debug"|"silly"} [level] Log level filter.
 * @property {number} [maxRows] Maximum number of returned rows.
 * @property {boolean} [includeGzip] Whether compressed historical logs should be included.
 * @property {boolean} [activeOnly] Whether only newly appended active-log lines should be read.
 * @property {boolean} [startNow] Return an empty result and a cursor at the current end of the log.
 * @property {number} [since] Only include timestamps after this server-side Unix timestamp in milliseconds.
 * @property {Partial<SearchCursor>} [cursor] Previous active-log read position.
...
async function streamLogLines(filePath, isGzip, startOffset, onLine, endOffset) {
    ...
    const input = fsSync.createReadStream(
        filePath,
 ...
 * @returns {Promise<any>} Search response and active-log cursor metadata.
 */
async function searchLogs(options) {
    ...
    const emitDiagnostics = () => {
        ...
        debug(
            `Log search start: dir=${logDirectory}, hours=${normalizedHours}, level=${effectiveLevel}, includeGzip=${effectiveIncludeGzip}, activeOnly=${!!activeOnly}, maxRows=${normalizedMaxRows}`,
        );
        debug(
    ...
    if (startNow) {
        ...
        const stats = activeFile ? await fs.stat(path.join(logDirectory, activeFile)) : null;
        return {
            ok: true,
            rows: [],
            total: 0,
    ...
    if (activeOnly) {
        ...
        let snapshotStats;
        try {
            snapshotStats = await fs.stat(fullPath);
        } catch {
            emitDiagnostics();
        ...
        if (activeFile) {
            try {
                activeSnapshotStats = await fs.stat(path.join(logDirectory, activeFile));
            } catch {
