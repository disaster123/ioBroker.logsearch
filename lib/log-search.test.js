...
describe("log-search utility", () => {
    ...
    it("starts from the server time without reading historical contents and polls only new entries", async () => {
        ...
        const readStream = require("sinon").spy(fsSync, "createReadStream");
        let start;
        try {
            start = await searchLogs({ logDirectory: tempDir, startNow: true, now: fixedNow });
            expect(readStream.called).to.equal(false);
        } finally {
            readStream.restore();
        }
        ...
        const poll = await searchLogs({ logDirectory: tempDir, activeOnly: true, cursor: start.cursor, since: start.since, now: later });
        expect(poll.rows.map(row => row.message)).to.deep.equal(["new entry"]);
        const resync = await searchLogs({ logDirectory: tempDir, since: start.since, now: later });
        expect(resync.rows.map(row => row.message)).to.deep.equal(["new entry"]);
        const history = await searchLogs({ logDirectory: tempDir, now: later });
    ...
    it("keeps the start boundary across filters, gzip history and log rotation", async () => {
        ...
        await fs.writeFile(path.join(tempDir, "iobroker.2026-05-27.log.gz"), zlib.gzipSync(`${before} - error: host.a match old\n`));
        await fs.writeFile(path.join(tempDir, "iobroker.current.log"), `${before} - error: host.a match old\n${after} - error: host.a match new\n${after} - info: host.a match info\n`);
        /** @type {import('./log-search').SearchLogsOptions} */
        const options = { logDirectory: tempDir, since: fixedNow.getTime(), now: fixedNow.getTime() + 2000, level: "error", searchText: "match" };
        const resync = await searchLogs(options);
        expect(resync.rows.map(row => row.message)).to.deep.equal(["match new"]);
        const rotated = await searchLogs({ ...options, activeOnly: true, cursor: { file: "iobroker.2026-05-26.log", byteOffset: 10000 } });
    ...
    it("keeps polling when started before any log file exists", async () => {
        const start = await searchLogs({ logDirectory: tempDir, startNow: true, now: fixedNow });
        expect(start.cursor).to.be.an("object");
        const empty = await searchLogs({ logDirectory: tempDir, since: start.since, activeOnly: true, cursor: start.cursor, now: fixedNow });
        expect(empty.cursor).to.be.an("object");
        const resync = await searchLogs({ logDirectory: tempDir, since: start.since, now: fixedNow });
        ...
        const poll = await searchLogs({ logDirectory: tempDir, since: start.since, activeOnly: true, cursor: empty.cursor, now: later });
    ...
    it("rejects invalid start boundaries instead of showing old entries", async () => {
        for (const since of [NaN, Infinity, -1]) {
            const result = await searchLogs({ logDirectory: tempDir, since });
    ...
    it("should return duplicate physical log lines with distinct row ids", async () => {
        ...
        const result = await searchLogs({ logDirectory: tempDir, searchText: "duplicate payload", now: fixedNow });

        expect(result.ok).to.equal(true);
        expect(result.rows).to.have.length(2);
        expect(result.rows.map(row => row.message)).to.deep.equal(["duplicate payload", "duplicate payload"]);
        expect(result.rows.map(row => row.rowId)).to.deep.equal(["iobroker.current.log:1", "iobroker.current.log:2"]);
    });
    ...
    it("should stream a normal text log and return a cursor", async () => {
        ...
        expect(result.cursor).to.include({ file: "iobroker.current.log", byteOffset: stats.size, lineNumber: 1, size: stats.size });
    ...
    it("should continue row ids from a normal search cursor during activeOnly updates", async () => {
        ...
        const initialResult = await searchLogs({ logDirectory: tempDir, now: fixedNow });
        expect(initialResult.ok).to.equal(true);
        expect(initialResult.cursor.lineNumber).to.equal(3);

        await fs.appendFile(filePath, `${thirdTs} - info: host.a third line\n`, "utf8");
        const updateResult = await searchLogs({
            logDirectory: tempDir,
            activeOnly: true,
        ...
        expect(updateResult.ok).to.equal(true);
    ...
    it("should keep activeOnly cursor at the pre-read snapshot when the active log grows during reading", async () => {
        ...
        fsSync.createReadStream = function patchedCreateReadStream(readPath, options) {
        ...
        const secondResult = await searchLogs({
            logDirectory: tempDir,
            activeOnly: true,
    ...
    it("should keep normal search cursor at the active log snapshot when the file grows during reading", async () => {
        ...
        let initialResult;
        try {
            initialResult = await searchLogs({ logDirectory: tempDir, searchText: "normal line", now: fixedNow });
        } finally {
            fsSync.createReadStream = originalCreateReadStream;
        }
        ...
        expect(appendedDuringRead).to.equal(true);
        expect(initialResult.ok).to.equal(true);
        expect(initialResult.rows.map(row => row.message)).to.deep.equal(["second normal line", "first normal line"]);
    ...
    it("should not read activeOnly lines when cursor byteOffset is at current file size", async () => {
        ...
        const result = await searchLogs({
            logDirectory: tempDir,
            activeOnly: true,
            cursor: { file: "iobroker.current.log", byteOffset: stats.size, lineNumber: /** @type {any} */ ("invalid"), size: stats.size, mtimeMs: stats.mtimeMs },
            now: fixedNow,
        });
    ...
    it("should stop reading older normal search files once maxRows plus one matches are found", async () => {
        ...
        await fs.writeFile(path.join(tempDir, "iobroker.2026-05-26.log"), "2026-05-26 11:00:00.000 - info: host.a older hit", "utf8");
        ...
        expect(debugMessages.some(message => message.startsWith("Log search file: name=iobroker.2026-05-26.log"))).to.equal(false);
    ...
    it("should continue reading older normal search files until maxRows plus one matches are found", async () => {
        await fs.writeFile(path.join(tempDir, "iobroker.current.log"), "2026-05-27 11:59:00.000 - info: host.a newest only", "utf8");
        await fs.writeFile(path.join(tempDir, "iobroker.2026-05-26.log"), "2026-05-26 11:00:00.000 - info: host.a older processed", "utf8");
        ...
        expect(debugMessages.some(message => message.startsWith("Log search file: name=iobroker.2026-05-26.log"))).to.equal(true);
        const doneMessage = debugMessages.find(message => message.startsWith("Log search done:"));
