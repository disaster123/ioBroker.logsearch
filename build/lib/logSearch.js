...
exports.getActiveLogFile = getActiveLogFile;
...
const LOG_LINE_RE = /^(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}(?:\.\d{1,3})?)\s+-\s+(error|warn|info|debug|silly):\s+([^\s]+)(?:\s+\((\d+)\))?\s+(.*)$/i;
function stripAnsiCodes(value) {
    return value.replace(ANSI_ESCAPE_RE, '');
}
function createFileDiagnostic(name) {
    return {
        name,
        lines: 0,
        parsed: 0,
 ...
 */
function getFileDayRange(name, datedRe) {
 ...
 */
async function getActiveLogFile(location, entries) {
    const dirEntries = entries || (await (0, promises_1.readdir)(location.directory, { withFileTypes: true }));
    const activeName = (0, logDirectory_1.getActiveLogFileName)(location);
    const current = dirEntries.find(entry => entry.isFile() && entry.name === activeName);
    if (current) {
        return current.name;
    }
    const datedRe = (0, logDirectory_1.getDatedLogFileRegExp)(location);
    const datedFiles = [];
...
function matchesFilters(parsed, context, fileDiagnostic, addUnparsedSample, originalLine) {
    if (!parsed || !types_1.LOG_LEVELS.includes(parsed.level)) {
...
async function streamLogLines(filePath, isGzip, startOffset, onLine, endOffset) {
    ...
    const input = (0, node_fs_1.createReadStream)(filePath, streamOptions && Object.keys(streamOptions).length ? streamOptions : undefined);
    const stream = isGzip ? input.pipe((0, node_zlib_1.createGunzip)()) : input;
    const lineReader = (0, node_readline_1.createInterface)({ input: stream, crlfDelay: Infinity });
    try {
...
async function searchLogs(options) {
    ...
    const { location, searchText = '', hours = 6, level = 'all', maxRows = 500, activeOnly = false, startNow = false, since, cursor, now, debugLog, } = options;
    ...
    const effectiveLevel = types_1.LOG_LEVELS.includes(level) ? level : 'all';
    ...
    const datedRe = (0, logDirectory_1.getDatedLogFileRegExp)(location);
    const activeLogName = (0, logDirectory_1.getActiveLogFileName)(location);
    ...
    const emitDiagnostics = () => {
        ...
        debug(`Log search start: dir=${logDirectory}, prefix=${location.prefix}, ext=${location.extension}, source=${location.source}, hours=${normalizedHours}, level=${effectiveLevel}, includeGzip=${effectiveIncludeGzip}, activeOnly=${!!activeOnly}, maxRows=${normalizedMaxRows}`);
    ...
    let files;
    try {
        files = await (0, promises_1.readdir)(logDirectory, { withFileTypes: true });
    }
    catch (error) {
    ...
    if (startNow) {
        ...
        const stats = activeFile ? await (0, promises_1.stat)((0, node_path_1.join)(logDirectory, activeFile)) : null;
        return {
            ok: true,
            rows: [],
            total: 0,
    ...
    let responseCursor = since === undefined ? null : createCursor('', null);
    ...
    if (activeOnly) {
        ...
        const fullPath = (0, node_path_1.join)(logDirectory, activeFile);
        let snapshotStats;
        try {
            snapshotStats = await (0, promises_1.stat)(fullPath);
        }
        catch {
            emitDiagnostics();
        ...
        if (debug) {
            debug(`Log search active cursor: start_offset=${startOffset}, snapshot_size=${snapshotSize}, cursor_offset=${Number(cursor?.byteOffset) || 0}`);
        }
        ...
        let activeSnapshotStats = null;
        if (activeFile) {
            try {
                activeSnapshotStats = await (0, promises_1.stat)((0, node_path_1.join)(logDirectory, activeFile));
            }
            catch {
        ...
        for (const name of candidates) {
            const fullPath = (0, node_path_1.join)(logDirectory, name);
            const fileDiagnostic = createFileDiagnostic(name);
            const isGzip = name.endsWith('.gz');
            const snapshotEndOffset = !isGzip && name === activeFile && activeSnapshotStats ? activeSnapshotStats.size - 1 : undefined;
            try {
                await streamLogLines(fullPath, isGzip, 0, line => {
                    fileDiagnostic.lines += 1;
                    if (!line.trim()) {
                        return;
                    }
        ...
        if (activeFile && activeSnapshotStats) {
            const activePath = (0, node_path_1.join)(logDirectory, activeFile);
            try {
